...
async def create_company(
    db: AsyncSession, data: CompanyCreate, logo_file: UploadFile | None = None
):
    ...
    if logo_file:
        upload_result = CloudinaryClient.upload_image(logo_file.file, folder="company")
...
async def save_trello_token(db: AsyncSession, company_id: str, token: str):
    company = await company_repo.get_company_by_id(db, company_id)
    if not company:
        raise HTTPException(status_code=404, detail="Company not found")
