...
class UtteranceService:
    ...
    async def get_resolved_user_id_is_null(self, meeting_id: uuid.UUID) -> List[UtteranceResponse]:
        return await self.utterance_repository.get_resolved_user_id_is_null_by_meeting_id(meeting_id)
    
    async def get_all_utterances_by_meeting_id(self, meeting_id: uuid.UUID) -> List[UtteranceResponse]:
        return await self.utterance_repository.get_all_utterances_by_meeting_id(meeting_id)
